...
const availablePorts = [5244, 1001, 1002]
const availableTime = [
  // From 6:30 to 7:00
  { start: '6:30', end: '7:00' },
  // From 12:00 to 12:40
...
const isAvailableTime = availableTime.some(({ start, end }) => {
  const now = dayjs()
  const startTime = dayjs(now).set('hour', Number(start.split(':')[0])).set('minute', Number(start.split(':')[1]))
  const endTime = dayjs(now).set('hour', Number(end.split(':')[0])).set('minute', Number(end.split(':')[1]))
