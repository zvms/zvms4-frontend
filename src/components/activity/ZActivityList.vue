      ...
      >
        ...
        <ElTableColumn
          ...
          :filters="
            statusFilter
              .filter(
                (x) => role === 'mine' || (x !== 'rejected' && x !== 'draft')
              )
              .map((x) => ({
        ...
        >
          <template #default="{ row }">
            ...
            <ZActivityStatus
              ...
              :modifiable="
                (role === 'class' &&
                  (row.type === 'social' || row.type === 'scale')) ||
                user.position.includes('admin') ||
                user.position.includes('department')
              "
              :activity="row"
        ...
        <ElTableColumn fixed="right">
          ...
          <template #default="{ row }">
            <ElButton
              ...
              type="primary"
              @click="
                router.push(`/activity/details/${row._id}/impression/${role}`)
              "
            >
            ...
            <ElButton
              ...
              v-else-if="
                ...
                row.members.filter((x: ActivityMember) => x.status === 'pending').length > 0 || role === 'class'
              "
              type="danger"
              text
              bg
