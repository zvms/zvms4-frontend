...
<template>
  <div>
    ...
    <ElResult v-if="incorrect" icon="error" title="Cannot find the word."> </ElResult>
    <div v-else>
        ...
        >
          ...
          <li v-for="item in meaning.definitions" :key="item.definition">
            <span v-if="item.definition"
              >Definition: <span class="font-serif">{{ item.definition }}</span
              ><br
            /></span>
            <span v-if="item.example"
              >Example: <span class="font-serif">{{ item.example }}</span
              ><br
            /></span>
            <span v-if="item.synonyms.length != 0"
              >Synonyms: {{ item.synonyms.join(', ') }}<br
            /></span>
            <span v-if="item.antonyms.length != 0"
              >Antonyms: {{ item.antonyms.join(', ') }}<br
            /></span>
          </li>
        </ElCard>
      </ElCard>
