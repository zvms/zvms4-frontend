...
async function getMyNotifications(id: string) {
  const result = (await axios(`/user/${id}/notification`)).data as Response<BroadcastInstance[]>
...
async function getNotifications() {
  ...
  const result = (await axios('/notification')).data as Response<BroadcastInstance[]>
  if (result.status === 'error') {
    ElNotification({
